import discord, asyncio, bot, sqlite3, pytz, sys, linecache, threading, configparser, signup, teams
...
# Lists/Dicts
team_finder = [
  {"fields":[{"name":"∘ **The bot will cancel the signup process if you cannot provide these links in time**","value":"**Please have these links ready: **\n- Steam profile link (ex. https://steamcommunity.com/id/...)\n- Faceit profile link (ex. https://www.faceit.com/en/players/...)\n","inline":False}],"title":"How to sign up","description":"1. *Make sure you have direct messages from server members enabled so you can receive notifications about being invited to a team*\n2. *Use the signup button and follow the signup process, and when captains are looking for members you will be sent an automated message from this bot if they invite you*\n3. *Once you receive a message for being invited to try out for a team, you will be informed of the team and the team captain that is adding you*","thumbnail":{"url":"https://www.chaoz.gg/wp-content/uploads/2021/12/ChaoZ-Discord-Bot-min.png"},"author":{"name":"CHAOZ Team finder","icon_url":"https://www.chaoz.gg/wp-content/uploads/2021/12/ChaoZ-Discord-Bot-min.png"},"color":13839167},
  {"title":"","description":"","thumbnail":{"url":"https://www.chaoz.gg/wp-content/uploads/2021/12/ChaoZ-Discord-Bot-min.png"},"color":13839167}
]
teams.team_find_embed = team_finder[0]
...
operator_role = int(gVars['operator_role'])
signup.signup_message = signup_embed
teams.panel_users = panel_users
teams.captain_role = captain_role
...
def teams_panel_embed(title, description="Select with buttons below", thumbnail="https://www.chaoz.gg/wp-content/uploads/2021/12/ChaoZ-Discord-Bot-min.png"):
  embed = discord.Embed(
    title=title,
    description=description
  ).set_thumbnail(url=thumbnail)
      ...
      try:
        for channel_id in player_find_channels:
          ...
          try:
            ...
            if message != None: await message.delete(); break
          except:
            PrintException()
      except: None
...
class MyClient(commands.Bot):
  ...
  async def on_ready(self):
    global server, log_channel, role_placeholder, signup_category, admin_role, operator_role
    ...
    admin_role = get(server.roles, id=admin_role)
    operator_role = get(server.roles, id=operator_role)
    signup_category = get(server.channels, id=signup_category)

    signup.server = server
    signup.signup_category = signup_category
    teams.role_placeholder = role_placeholder
    teams.admin_role = admin_role
    teams.operator_role = operator_role
...
class MyCommands(commands.Cog):
  ...
  async def teams(self, ctx):
    ...
    message = await ctx.channel.send(embed=teams_panel_embed("**Please select an option**"))
    await message.edit(view=PanelView(message, self.bot, ctx.author))
